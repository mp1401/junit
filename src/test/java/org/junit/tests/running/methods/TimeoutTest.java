...
public class TimeoutTest {
    ...
    public static class MyTimeoutHandler implements TimeoutHandler {
        ...
        public Exception handleTimeout(final Thread thread) {
            final String prefix = "[" + getClass().getSimpleName() + "] ";
            log.warning(prefix
                    + "Test ran into timeout, here is a full thread dump:\n"
                    + getFullThreadDump());
            return new Exception(
                    prefix
                            + "Appears to be stuck => Full thread dump is logged as warning");
        }

        // TODO: made temporarily static to allow thread dump in teardown
        // showing surviving threads from other tests
        // private String getFullThreadDump() {
        public static String getFullThreadDump() {
            final StringBuilder sb = new StringBuilder();

            // TODO: ThreadMXBean provides interesting thread dump information
            // (locks, monitors, synchronizers) only with Java >= 1.6

            // First try ThreadMXBean#findMonitorDeadlockedThreads():
            final ThreadMXBean threadMxBean = ManagementFactory
                    .getThreadMXBean();
            final long[] deadlockedThreadIds = threadMxBean
            ...
            if (deadlockedThreadIds != null) {
                ...
                for (final ThreadInfo threadInfo : threadInfos) {
                    sb.append("\n\t" + threadInfo.getThreadName() + " Id="
                            + threadInfo.getThreadId() + " Lock name="
                            + threadInfo.getLockName() + " Lock owner Id="
                            + threadInfo.getLockOwnerId() + " Lock owner name="
            ...
            sb.append("\n");
            for (final Entry<Thread, StackTraceElement[]> threadEntry : allStackTraces
                    .entrySet()) {
                sb.append("\n" + threadToHeaderString(threadEntry.getKey()));

                final StackTraceElement[] stackTraces = threadEntry.getValue();
                for (int i = 0; i < stackTraces.length; i++) {
                    final StackTraceElement ste = stackTraces[i];
        ...
        private static String threadToHeaderString(final Thread thread) {
            final StringBuilder sb = new StringBuilder("\"" + thread.getName()
                    + "\"" + " Id=" + thread.getId() + " Daemon="
                    + thread.isDaemon() + " State=" + thread.getState()
                    + " Priority=" + thread.getPriority() + " Group="
    ...
    public static abstract class AbstractLockedWithDeadlockTest {
        ...
        public void teardown() throws InterruptedException {
            ...
            // TODO: temporary thread dumping in teardown to show surviving
            // threads from other tests
    ...
    public static class LockedWithDeadlockTest extends
            AbstractLockedWithDeadlockTest {
        @Rule
        public TestRule globalTimeout = new Timeout(100, TimeUnit.MILLISECONDS)
                .customTimeoutHandler(new MyTimeoutHandler());
    }
    ...
    public void timeoutFailureMultithreadedDeadlockWithFullDump()
        ...
        assertThat(
                ...
                containsString("[MyTimeoutHandler] Appears to be stuck => Full thread dump is logged as warning"));
    }

    public static class LockedWithDeadlockWithNoDedicatedTimeoutHandlerTest
            extends AbstractLockedWithDeadlockTest {
        @Rule
        public TestRule globalTimeout = new Timeout(100, TimeUnit.MILLISECONDS);
    }

    @Test
    public void timeoutFailureMultithreadedDeadlockWithFullDumpDueToGlobalTimeoutHandler()
            throws Exception {
        try {
            System.setProperty(
                    Timeout.TIMEOUT_HANDLER_CLASS_NAME_PROPERTY_NAME,
                    "org.junit.tests.running.methods.TimeoutTest$MyGlobalTimeoutHandler");

            final JUnitCore core = new JUnitCore();
            final Result result = core
                    .run(LockedWithDeadlockWithNoDedicatedTimeoutHandlerTest.class);
            assertEquals(1, result.getRunCount());
            assertEquals(2, result.getFailureCount());
            final Throwable exception[] = new Throwable[2];
            for (int i = 0; i < 2; i++)
            ...
            assertThat(
                    exception[1].getMessage(),
                    containsString("[MyGlobalTimeoutHandler] Appears to be stuck => Full thread dump is logged as warning"));
        } finally {
    ...
    /**
     * Because test case
     * {@link LockedWithDeadlockWithPartiallyDedicatedTimeoutHandlerTest#failure2()}
     * has a test method timeout, it gets a specific timeout rule.
     * <p>
     * TODO: green in Eclipse, but red in Ant.
     */
    @Test
    public void timeoutFailureMultithreadedDeadlockWithPartiallyFullDumpDueToGlobalTimeoutHandler()
            throws Exception {
        try {
            System.setProperty(
                    Timeout.TIMEOUT_HANDLER_CLASS_NAME_PROPERTY_NAME,
            ...
            final JUnitCore core = new JUnitCore();
            final Result result = core
                    .run(LockedWithDeadlockWithPartiallyDedicatedTimeoutHandlerTest.class);
            assertEquals(2, result.getRunCount());
            assertEquals(4, result.getFailureCount());
            final Throwable exception[] = new Throwable[4];
            // TODO: in Ant execution this test fails; and there are quite some
            // deadlocked threads surviving/around from other tests:
            ...
            assertThat(exception[0].getMessage(),
                    containsString("test timed out after 70 milliseconds"));
            assertThat(stackForException(exception[0]),
                    containsString("Thread.join"));
            assertThat(
                    exception[1].getMessage(),
            ...
            assertThat(exception[2].getMessage(),
                    containsString("test timed out after 100 milliseconds"));
            assertThat(stackForException(exception[2]),
                    containsString("Thread.join"));
            assertThat(
                    exception[3].getMessage(),
                    containsString("[MyGlobalTimeoutHandler] Appears to be stuck => Full thread dump is logged as warning"));
        } finally {
            System.clearProperty(Timeout.TIMEOUT_HANDLER_CLASS_NAME_PROPERTY_NAME);
        }
