...
public class CustomTimeoutTest {
    ...
    public static class CustomTimeoutHandler {
        ...
        public Exception handleTimeout(final Thread thread) {
            ...
            return new Exception(
                    prefix
                            + "Appears to be stuck due to running into timeout. Here could be some more custom failure context...");
        }
    ...
    public static class CustomFailOnTimeout extends FailOnTimeout {
        ...
        public CustomFailOnTimeout(final Statement base, final long timeout,
        ...
        protected List<Throwable> createAdditionalTimeoutExceptions(
                final Thread thread) {
            final List<Throwable> exceptions = super
                    .createAdditionalTimeoutExceptions(thread);
            final Throwable handleTimeout = handler.handleTimeout(thread);
    ...
    public static class CustomTimeout extends Timeout {
        ...
        public CustomTimeout(final int timeout, final TimeUnit unit) {
            this(timeout, unit, false);
        }

        public CustomTimeout(final int timeout, final TimeUnit unit,
                final boolean lookForStuckThread) {
        ...
        public Statement apply(final Statement base,
            ...
            return new CustomFailOnTimeout(base, getTimeout(), getTimeUnit(),
                    isLookForStuckThread());
        }
    }
    ...
    public static class InfiniteLoopTest {
        ...
        @Rule
        public TestRule globalTimeout = new CustomTimeout(100,
    ...
    public void infiniteLoop() throws Exception {
        ...
        assertThat(
                ...
                containsString("[CustomTimeoutHandler] Appears to be stuck due to running into timeout. Here could be some more custom failure context..."));
    }

    // --------------------------------------------------------------------------------------------------
    // Below is the additional code for test scenario when the timeout is also
    // looking for stuck threads:
    ...
    public void infiniteLoopWithLookForStuckThread() throws Exception {
        ...
        final Result result = core
                .run(InfiniteLoopWithLookForStuckThreadTest.class);
        assertEquals(1, result.getRunCount());
        assertEquals(3, result.getFailureCount());
        final Throwable exception[] = new Throwable[3];
        for (int i = 0; i < 3; i++)
            exception[i] = result.getFailures().get(i).getException();
        assertThat(exception[0].getMessage(),
                containsString("test timed out after 100 milliseconds"));
