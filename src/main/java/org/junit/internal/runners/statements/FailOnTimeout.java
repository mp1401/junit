...
public class FailOnTimeout extends Statement {
    ...
    private Exception createTimeoutException(final Thread thread) {
        ...
        if (timeoutHandler != null) {
            // For the sake of convenience just allow adding another exception
            // by the custom timeout handler
            final Exception timeoutHandlerException = timeoutHandler
     ...
     * {@link Timeout#apply(Statement, org.junit.runner.Description)}, but this
     * would not have been used for timeouts specified for single test cases.
     */
    private TimeoutHandler lookForGlobalTimeoutHandler() {
        // TODO: due to using reflection, the global timeout handler should be
        // cached once it is
        // there, and used as long as the system property is still set?
        TimeoutHandler handler = null;
        final String timeoutHandlerClassName = System
                .getProperty(Timeout.TIMEOUT_HANDLER_CLASS_NAME_PROPERTY_NAME);
        if (timeoutHandlerClassName != null) {
            try {
                System.err.println("FOUND");
                final Class<?> timeoutHandlerClass = Class
                        .forName(timeoutHandlerClassName);
                final Object handlerInstance = timeoutHandlerClass
                        .newInstance();
                ...
                throw new RuntimeException(
                        String.format(
                                "Failed to find timeout handler class '%s' specified via system property",
                                timeoutHandlerClassName), e);
            } catch (final InstantiationException e) {
                // TODO: or throw InitializationError?
                throw new RuntimeException(String.format(
                        "Failed to instantiate timeout handler class '%s'",
                        timeoutHandlerClassName), e);
            } catch (final IllegalAccessException e) {
                // TODO: or throw InitializationError?
                throw new RuntimeException(
                        String.format(
                                "Failed to access timeout handler class '%s' during instantion",
                                timeoutHandlerClassName), e);
            } catch (final ClassCastException e) {
                // TODO: or throw InitializationError?
                throw new RuntimeException(String.format(
                        "Failed to cast timeout handler class '%s' to '%s'",
                        timeoutHandlerClassName, TimeoutHandler.class), e);
            }
        }
        return handler;
    }
