...
public class FailOnTimeout extends Statement {
    ...
    public FailOnTimeout(final Statement originalStatement, final long timeout,
            ...
            final boolean fullThreadStackDump) {
    ...
    private Exception createTimeoutException(final Thread thread) {
        ...
        final Thread stuckThread = fLookForStuckThread ? getStuckThread(thread)
                : null;
        final Exception currThreadException = new Exception(String.format(
                "test timed out after %d %s", fTimeout, fTimeUnit.name()
                        .toLowerCase()));
        if (stackTrace != null) {
        ...
        // TODO: would it make sense to make this optional, i.e. if stuckThread
        // != null, then skip the full thread dump?
        if (fFullThreadStackDump) {
            // For the sake of convenience just add the full thread dump
            // directly to the failure message: this can really
            // hurt when getting long :-(
            final Exception fullThreadDumpException = new Exception(
    ...
    private String threadToHeaderString(final Thread thread) {
